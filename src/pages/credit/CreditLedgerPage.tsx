  ...
  return (
    <div style={{ padding: '12px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
      ...
      <div style={{ background: 'var(--bg-surface)', borderRadius: '8px', border: '1px solid var(--border-default)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px', borderBottom: '1px solid var(--bg-tertiary)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <h3 style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>Debtor Registry</h3>
            <span style={{ fontSize: '10px', color: 'var(--text-muted)', background: 'var(--bg-tertiary)', padding: '2px 6px', borderRadius: '999px' }}>{filteredEntries.length}</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ fontSize: '10px', border: '1px solid var(--border-default)', borderRadius: '4px', padding: '4px 8px', background: 'var(--bg-surface)', color: 'var(--text-primary)' }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', minWidth: '480px', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ fontSize: '10px', color: 'var(--text-muted)', borderBottom: '1px solid var(--bg-tertiary)' }}>
            ...
            <tbody>
              {filteredEntries.map((entry: any) => (
                <tr key={entry.id} style={{ borderBottom: '1px solid var(--bg-secondary)' }}>
                  <td style={{ padding: '12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <div style={{ width: '28px', height: '28px', background: 'var(--bg-tertiary)', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <span style={{ fontSize: '10px', fontWeight: 700, color: 'var(--text-secondary)' }}>{entry.customer_name?.split(' ').map((n: string) => n[0]).join('') || '??'}</span>
                      </div>
                      <div>
                        <p style={{ fontSize: '12px', fontWeight: 500, color: 'var(--text-primary)', margin: 0 }}>{entry.customer_name}</p>
                        <p style={{ fontSize: '10px', color: 'var(--text-muted)', margin: 0 }}>{entry.customer_code}</p>
                      </div>
                    </div>
                  </td>
                  <td style={{ padding: '12px', textAlign: 'right', fontSize: '12px', fontWeight: 600, color: 'var(--text-primary)', whiteSpace: 'nowrap' }}>{format(entry.balance || 0)}</td>
                  <td style={{ padding: '12px' }}>
                    <p style={{ fontSize: '10px', color: 'var(--text-primary)', whiteSpace: 'nowrap', margin: 0 }}>{entry.last_payment}</p>
                    {entry.last_payment_amount && <p style={{ fontSize: '10px', color: 'var(--text-muted)', margin: 0 }}>{format(entry.last_payment_amount)} PARTIAL</p>}
                  </td>
                  <td style={{ padding: '12px' }}>
                  ...
                  <td style={{ padding: '12px', textAlign: 'right', position: 'relative' }}>
                    <button onClick={() => setOpenMenu(openMenu === entry.id ? null : entry.id)} style={{ color: 'var(--text-placeholder)', background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }}>•••</button>
                    {openMenu === entry.id && (
                      <div ref={menuRef} style={{ position: 'absolute', right: 0, top: '100%', background: 'var(--bg-surface)', borderRadius: '8px', border: '1px solid var(--border-default)', boxShadow: 'var(--shadow-menu)', zIndex: 999, minWidth: '120px', overflow: 'hidden' }}>
                        <button onClick={() => { setShowPayForm(entry); setOpenMenu(null) }} style={{ width: '100%', textAlign: 'left', padding: '8px 12px', fontSize: '12px', color: 'var(--text-primary)', background: 'none', border: 'none', cursor: 'pointer' }}>Record Payment</button>
                        <button onClick={() => handleUpdateStatus(entry, entry.status === 'active' ? 'overdue' : 'active')} style={{ width: '100%', textAlign: 'left', padding: '8px 12px', fontSize: '12px', color: 'var(--text-primary)', background: 'none', border: 'none', cursor: 'pointer' }}>
              ...
              {filteredEntries.length === 0 && !loading && (
                <tr><td colSpan={5} style={{ padding: '24px', textAlign: 'center', color: 'var(--text-placeholder)', fontSize: '12px' }}>{statusFilter === 'all' ? 'No credit entries yet' : 'No entries match this status'}</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {showForm && (
        <div style={modalBackdrop} onClick={() => setShowForm(false)}>
          <div style={modalCard} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontSize: '16px', fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>New Credit Entry</h3>
            <div>
              <label style={{ fontSize: '12px', fontWeight: 500, color: 'var(--text-label)', marginBottom: '4px', display: 'block' }}>Customer Name</label>
            ...
            <div>
              <label style={{ fontSize: '12px', fontWeight: 500, color: 'var(--text-label)', marginBottom: '4px', display: 'block' }}>Customer Code (optional)</label>
            ...
            <div>
              <label style={{ fontSize: '12px', fontWeight: 500, color: 'var(--text-label)', marginBottom: '4px', display: 'block' }}>Initial Balance</label>
      ...
      {showPayForm && (
        <div style={modalBackdrop} onClick={() => setShowPayForm(null)}>
          <div style={modalCard} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontSize: '16px', fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>Record Payment</h3>
            <p style={{ fontSize: '13px', color: 'var(--text-secondary)', margin: 0 }}>{showPayForm.customer_name} — Current balance: <strong>{format(showPayForm.balance || 0)}</strong></p>
            <div>
              <label style={{ fontSize: '12px', fontWeight: 500, color: 'var(--text-label)', marginBottom: '4px', display: 'block' }}>Payment Amount</label>
