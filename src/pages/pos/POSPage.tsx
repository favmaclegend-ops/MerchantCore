...
export function POSPage() {
  ...
  if (loadingProducts) return <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-placeholder)' }}>Loading...</div>
    ...
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '16px', padding: '12px', flex: '1', height: '100%' }}>
       ...
       { 
        ...
        <div style={{ background: 'var(--bg-surface)', borderRadius: '8px', border: '1px solid var(--border-default)', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px', maxHeight: bp.lg ? '240px' : '200px' }}>
            {cart.length === 0 ? (
              <p style={{ fontSize: '12px', color: 'var(--text-placeholder)', textAlign: 'center', padding: '24px 0', margin: 0 }}>Empty</p>
            ) : (
              ...
              cart.map(item => (
                <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                  <div style={{ width: '28px', height: '28px', background: 'var(--bg-tertiary)', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <span style={{ fontSize: '10px', fontWeight: 700, color: 'var(--text-secondary)' }}>{item.name[0]}</span>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: '10px', fontWeight: 500, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', margin: 0 }}>{item.name}</p>
                    <p style={{ fontSize: '10px', color: 'var(--text-muted)', margin: 0 }}>{format(item.price)} × {item.quantity}</p>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
                    <button onClick={() => updateQuantity(item.id, -1)} style={{ width: '20px', height: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '4px', background: 'var(--bg-tertiary)', border: 'none', cursor: 'pointer' }}>
                  ...
                  <p style={{ fontSize: '12px', fontWeight: 700, color: 'var(--text-primary)', margin: 0, minWidth: '48px', textAlign: 'right' }}>{format(item.price * item.quantity)}</p>
          ...
          <div style={{ padding: '12px', borderTop: '1px solid var(--bg-tertiary)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', marginBottom: '4px' }}>
              ...
              <span style={{ color: 'var(--text-primary)' }}>{format(subtotal)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', marginBottom: '4px' }}>
              <span style={{ color: 'var(--text-muted)' }}>Tax (5%)</span>
              <span style={{ color: 'var(--text-primary)' }}>{format(tax)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', fontWeight: 700, paddingTop: '6px', borderTop: '1px solid var(--bg-tertiary)' }}>
              <span>Total</span>
          ...
          <div style={{ padding: '12px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px' }}>
            {paymentButtons.map(pb => {
              ...
              return (
                <button key={pb.label} onClick={() => setPaymentMethod(pb.label)} style={{
                  ...
                  background: isActive ? 'var(--bg-nav-active)' : 'var(--bg-secondary)',
                  color: isActive ? 'var(--text-primary)' : 'var(--text-secondary)',
                  border: isActive ? 'none' : '1px solid var(--border-default)', borderRadius: '4px', cursor: 'pointer',
                }}>
            ...
            <button onClick={openLog} style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', padding: '6px 0',
              fontSize: '10px', fontWeight: 500, background: 'var(--bg-secondary)', border: '1px solid var(--border-default)',
          ...
          <div style={{ padding: '0 12px 12px' }}>
            <button onClick={handleCheckout} disabled={cart.length === 0 || checkingOut} style={{
              width: '100%', padding: '8px 0', fontSize: '12px', fontWeight: 600,
              color: 'var(--text-secondary)', background: cart.length === 0 ? 'var(--text-placeholder)' : 'var(--bg-nav-active)',
      ...
      {showLog && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '16px' }} onClick={() => setShowLog(false)}>
          <div style={{ background: 'var(--bg-surface)', borderRadius: '12px', padding: '24px', width: '100%', maxWidth: '500px', maxHeight: '80vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '12px' }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontSize: '16px', fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>Recent Transactions</h3>
            {transactions.length === 0 && <p style={{ fontSize: '12px', color: 'var(--text-placeholder)', textAlign: 'center' }}>No transactions yet</p>}
            {transactions.slice(0, 20).map((tx: any) => (
              <div key={tx.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--bg-tertiary)', fontSize: '12px' }}>
                <div>
