...
const emptyProductForm: ProductForm = { name: '', sku: '', category: '', price: '', stock: '' }
...
export function InventoryTracking({ products, reload, notify, orgUser }: { products: OrgProduct[]; reload: () => void; notify: (msg: string) => void; orgUser: OrgMember }) {
  ...
  const submit = async () => {
    ...
    try {
      if (editing) {
        await api.org.updateProduct(editing.id, { name: form.name.trim(), sku: form.sku.trim(), category: form.category, price, stock })
        notify(`${form.name.trim()} was updated`)
      } else {
        await api.org.createProduct({ name: form.name.trim(), sku: form.sku.trim(), category: form.category, price, stock })
