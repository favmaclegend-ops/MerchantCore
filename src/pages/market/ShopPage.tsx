        ...
        <>
            <div style={{display: 'flex', width: '100%', height: '100%', padding: bp.sm ? '.25rem' : '.5rem', flexDirection: 'column', maxWidth: '900px', gap: '1rem', overflowY: 'auto', overflowX: 'hidden'}}>
                <div style={{display: 'flex', flexDirection: 'column', width: '100%', position: 'relative', }}>
                    ...
                    <div style={{width: '100%', maxWidth: '1500px', borderRadius: '1rem 1rem 0rem 0rem', height: bp.sm ? '6rem' : '10rem', background: '#050505', position: 'absolute', overflow: 'hidden'}}>
                        <img src={shop.shopProfileImage} style={{objectFit: 'cover', objectPosition: 'center', width: '100%', height: '100%'}}/>
                    </div>

            
                    <div style={{display: 'flex', alignItems: 'flex-end', zIndex: '11', minHeight: bp.sm ? '9rem' : '15rem', gap: bp.sm ? '.5rem' : '1rem', minWidth: '0', width: '100%', paddingBlockEnd: bp.sm ? '.5rem' : '1rem'}}>

                        <div style={{ width: bp.sm ? '5rem' : '100%', maxWidth: '10rem', minWidth: bp.sm ? '3.5rem' : '4rem', aspectRatio: '1/1', borderRadius: '50%', background: 'blue', flex: '0 1 auto', overflow: 'hidden', boxShadow: '0 8px 24px rgba(0,0,0,.25)'}}>
                            <img src={shop.shopProfileImage} style={{objectFit: 'cover', objectPosition: 'center', width: '100%', height: '100%'}}/>
                        </div>
                    
                        <div style={{display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: bp.sm ? '.5rem' : '1rem', width: '100%', minWidth: '0'}}>

                            
                            <div style={{display: 'flex', flexDirection: 'column', minWidth: '0', overflow: 'hidden', paddingTop: '3rem', flex: '0 0 auto'}}>
                                <h1 style={{marginTop: '3rem', display: 'block', fontWeight: 'bolder', fontSize: bp.sm ? '1.1rem' : '1.5rem', lineHeight: 1.2, textWrap: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden', width: '100%'}}>
                                    {shop.shop_name}
                                </h1>
