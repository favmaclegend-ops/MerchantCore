...
export async function apiListMessages(threadId: string): Promise<{ thread: ChatApiThread; messages: ChatApiMessage[] }> {
...
export async function apiSendMessage(
  ...
  const res = await chatRequest<{ message: ChatApiMessage }>(`/chat/threads/${threadId}/messages`, {
    method: 'POST',
    body: JSON.stringify({ text: plaintext, thread_key: threadKey }),
  })
  return res.message
}
