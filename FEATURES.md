...
Admin) only · `[dev]` mock-backed until the backend ships.
...
| Role hierarchy | `src/data/organisations.ts` | `super-admin` → `admin` → `staff` |
...
| Users & roles management | `src/pages/users/` |
...
## 4. Finance & Accounting — `[org]` `[admin]` `[dev]`
...
## 5. Human Resources (HRM) — `[org]` `[admin]` `[dev]` · **NEW**
...
tracking, performance reviews and benefits:

| Sub-feature | Details |
|-------------|---------|
| **Employees** | Add/edit employees; statuses `probation → active → on-leave → terminated / retired`; department, job title, employment type, hire date, salary |
...
| **HRM Overview** | Headcount by status, monthly payroll, benefits cost, open reviews, hours logged |
...
- The **organisation workspace (incl. HRM) is mock-backed** in `localStorage`
