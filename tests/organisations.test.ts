...
describe('organisations (mock data layer)', () => {
  ...
  describe('loadOrganisations', () => {
    it('seeds the demo Sunrise Mart workspace', () => {
      ...
      expect(orgs[0].members).toHaveLength(5)
      expect(localStorage.getItem('merchant_org_data_version')).toBe('3')
    })
    ...
    it('reseeds when stored data is corrupt', () => {
      localStorage.setItem('merchant_org_data_version', '3')
  ...
  describe('member CRUD', () => {
    ...
    it('adds, updates and deletes members for the active org', () => {
      ...
      expect(admin.id).toBe('ADM-003')

      expect(loadOrganisations()[0].members).toHaveLength(7)
